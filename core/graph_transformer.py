...
def transform_graph_for_decode_always(G: nx.Graph, schemes: List[EncodingScheme], source):
    """
        将原始图 G 转换为用于 decode_always 多编码问题的扩展图 G" (在代码中我们称之为 G_prime)。
        - G: 原始网络图
        - schemes: 可用的编码方案列表
        返回: 扩展后的有向图
        """
    ...
    for node in G.nodes():
        if G.nodes[node]['type'] == 'switch':
            ...
            node_map[node] = de_nodes + ([fo_node] if fo_node is not None else [])
...
def transform_graph_for_forward_always(G: nx.Graph, schemes: List[EncodingScheme], dest) -> nx.DiGraph:
    """
    将原始图 G 转换为用于多编码问题的扩展图 G" (在代码中我们称之为 G_prime)。
    - G: 原始网络图
    - schemes: 可用的编码方案列表
